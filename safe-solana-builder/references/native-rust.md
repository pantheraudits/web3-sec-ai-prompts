...
// Step 2: Assign ownership back to System Program
account_to_close.assign(&system_program::ID);

// Step 3: Realloc to 0 data len
info.realloc(0, false)?;
```

- Skipping step 1 (zero lamports): attackers recover all lamports deposited for rent or other purposes.
- Skipping step 2 (reassigning owner): your program still "owns" a zero-balance account, wasting space.
- Skipping step 3 (realloc): rent must be deposited to maintain a zero-byte account.

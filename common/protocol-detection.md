...
| Solana (Rust/Anchor) | [`common/solana-patterns.md`](solana-patterns.md) | 20 vulnerability categories: account validation, PDA security, CPI safety, release-mode arithmetic, Token-2022, oracle manipulation, fee bypass, token dust DoS, state management, pool logic, timing bugs, mint integrity + framework-specific checks for Anchor and native Rust |
...
If the protocol is written in Rust/Anchor for Solana, **always** run `common/solana-patterns.md` alongside the standard review checklist. Solana has fundamentally different vulnerability classes than EVM (account validation model, PDA-based addressing, CPI trust boundaries, release-mode arithmetic without overflow checks).
